...
// These values should be used by modules that will not have access to the
// correct clock frequency values at build time.  For example, modules that
// are distributed as object files or libraries should use these values when
// determining timeouts, instead of the constants below.

extern DWORD   dwTicksPerSecond;
extern DWORD   dwTicksPerMillisecond;
...
#if defined(_PIC14E)
    #define TICKS_PER_SECOND        ((QWORD)((GetPeripheralClock()+4ull)/8ull))
#else
