 ...
 *                  HI-TECH PICC-18 PRO 9.93PL2 or higher
 * Company:         Microchip Technology, Inc.
 *
 * Software License Agreement
 *
 * Copyright © 2002-2012 Microchip Technology Inc.  All rights
 * reserved.
 *
...
void TickInit(void)
{
    dwTicksPerSecond        = TICKS_PER_SECOND;
    dwTicksPerMillisecond   = (TICKS_PER_SECOND + 500) / 1000ull;   // Add 500 for rounding.

#if defined ( _PIC14E )
    // Use Timer1 for PIC16 8 bit processors
    // Initialize the time
    TMR1H = 0;
    TMR1L = 0;
    ...
    T1CON = 0x31; // 16bit 1:8 prescaler
...
static void GetTickCopy(void)
{
    ...
    do
    {
        ...
        vTickReading[0] = TMR1L;
        vTickReading[1] = TMR1H;
        *((DWORD*)&vTickReading[2]) = dwInternalTicks; // pune tot ce e citit pana acum in vTickReading[2] - vTickReading[5]
