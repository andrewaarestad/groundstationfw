...
void Delay10us( UINT32 tenMicroSecondCounter )
{
        ...
        if ( tenMicroSecondCounter )
        {
            unsigned long sysClk = GetSystemClock();
            unsigned long t0;
            
            t0 = _CP0_GET_COUNT();
            while (_CP0_GET_COUNT() - t0 < (sysClk/200000)*tenMicroSecondCounter);
        }
...
void DelayMs( UINT16 ms )
{
        ...
        if ( ms )
        {
            ...
            t0 = _CP0_GET_COUNT();
            while (_CP0_GET_COUNT() - t0 < (sysClk/2000)*(unsigned long)ms);
        }
